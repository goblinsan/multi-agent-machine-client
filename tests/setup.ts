...
function ensureTmpCwd(opts?: any) {
  const cwd =
    opts && typeof opts === "object" && "cwd" in opts ? opts.cwd || "" : "";
  if (!cwd) return;
  const allowed = String(cwd).startsWith(os.tmpdir());
  if (!allowed) {
    throw new Error(
      `Test guard: git command attempted outside tmp dir. cwd=${cwd}`,
    );
  }
}
...
try {
  if (!vi.isMockFunction((childProcess as any).exec)) {
    vi.spyOn(childProcess as any, "exec").mockImplementation(
      (...args: any[]) => {
        const command = args[0];
        const options = args[1];
        if (typeof command === "string" && /\bgit\b/.test(command))
          ensureTmpCwd(options);
  ...
  if (!vi.isMockFunction((childProcess as any).execSync)) {
    vi.spyOn(childProcess as any, "execSync").mockImplementation(
      (...args: any[]) => {
        const command = args[0];
        const options = args[1];
        if (typeof command === "string" && /\bgit\b/.test(command))
  ...
  if (!vi.isMockFunction((childProcess as any).execFile)) {
    vi.spyOn(childProcess as any, "execFile").mockImplementation(
      (...args: any[]) => {
        const file = args[0];
  ...
  if (!vi.isMockFunction((childProcess as any).execFileSync)) {
    vi.spyOn(childProcess as any, "execFileSync").mockImplementation(
      (...args: any[]) => {
        const file = args[0];
        const options = args[2];
        if (file === "git") ensureTmpCwd(options);
  ...
  if (!vi.isMockFunction((childProcess as any).spawn)) {
    vi.spyOn(childProcess as any, "spawn").mockImplementation(
      (...args: any[]) => {
        const command = args[0];
        const options = args[2];
  ...
  if (!vi.isMockFunction((childProcess as any).spawnSync)) {
    vi.spyOn(childProcess as any, "spawnSync").mockImplementation(
      (...args: any[]) => {
        const command = args[0];
        const options = args[2];
        if (command === "git") ensureTmpCwd(options);
