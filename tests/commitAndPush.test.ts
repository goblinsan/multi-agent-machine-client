...
describe("Coordinator commit and push (integration-ish)", () => {
  it("handles file operations and git workflows without hanging", async () => {
    ...
    vi.spyOn(fileops, "applyEditOps").mockImplementation(
      async (_jsonText: string, _opts: any) => {
        await fs.mkdir(path.join(tmp, "src"), { recursive: true });
        await fs.writeFile(
          path.join(tmp, "src", "test.ts"),
    ...
    try {
      ...
      const timeoutPromise = new Promise<boolean>((_, reject) =>
        setTimeout(
          () => reject(new Error("Test timeout - workflow hanging")),
          100,
        ),
      );
