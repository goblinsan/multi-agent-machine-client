...
import { buildPersonaMessages, callPersonaModel } from '../src/personas/PersonaRequestHandler.js';
...
describe('PersonaRequestHandler', () => {
  ...
  it('builds messages with scan summary, dashboard, QA, planning, snippets, and extras', () => {
    ...
    expect(msgs.some(m => m.content.includes('Dashboard context'))).toBe(true);
    expect(msgs.some(m => m.content.includes('Latest QA Test Results'))).toBe(true);
    expect(msgs.some(m => m.content.includes('Previous Planning Iterations'))).toBe(true);
    expect(msgs.some(m => m.content.includes('Existing project files for reference'))).toBe(true);
    expect(msgs.some(m => m.content.includes('Extra guidance here'))).toBe(true);
  ...
  it('calls model and returns response content and duration', async () => {
    ...
    const res = await callPersonaModel({ persona: 'context', model: 'foo:bar', messages, timeoutMs: 1000 });
