...
describe("Phase 1: Variable Resolution in Artifact Paths", () => {
  ...
  describe("Simple Variable Resolution", () => {
    it("should resolve ${task.id} in plan_artifact path", async () => {
      ...
      expect(requestOpts.payload.plan_artifact).toBe(
        ".ma/tasks/42/03-plan-final.md",
      );
    ...
    it("should resolve ${task.id} in multiple artifact paths", async () => {
      ...
      expect(requestOpts.payload.plan_artifact).toBe(
        ".ma/tasks/99/03-plan-final.md",
      );
      expect(requestOpts.payload.qa_result_artifact).toBe(
        ".ma/tasks/99/05-qa-result.md",
      );
    });
  });
    ...
    it("should resolve ${milestone.slug} in artifact paths", async () => {
      ...
      const config = {
        ...
        config: {
          ...
          payload: {
            plan_artifact:
              ".ma/milestones/${milestone.slug}/tasks/${task.id}/plan.md",
      ...
      expect(requestOpts.payload.plan_artifact).toBe(
        ".ma/milestones/phase-1/tasks/10/plan.md",
      );
    ...
    it("should resolve deeply nested ${task.milestone.slug}", async () => {
      ...
      const config = {
        ...
        config: {
          ...
          payload: {
            artifact_path:
              ".ma/tasks/${task.id}/milestone-${task.milestone.slug}.md",
          },
        },
      };
      ...
      expect(requestOpts.payload.artifact_path).toBe(
        ".ma/tasks/5/milestone-sprint-2.md",
      );
    });
  });
  ...
  describe("Real-World Task Flow Scenarios", () => {
    ...
    it("should resolve task-flow.yaml qa_request payload", async () => {
      ...
      expect(requestOpts.payload.plan_artifact).toBe(
        ".ma/tasks/2/03-plan-final.md",
      );
      expect(requestOpts.payload.implementation).toBe(
        "Implementation complete",
      );
    });
  });
  ...
  describe("Fallback Behavior", () => {
    it("should preserve template if variable not found", async () => {
      ...
      expect(requestOpts.payload.milestone_artifact).toBe(
        ".ma/milestones/${milestone.id}/info.md",
      );
    });
  });
});
