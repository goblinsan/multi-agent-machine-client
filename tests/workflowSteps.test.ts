...
} from "../src/workflows/helpers/workflowAbort.js";
import {
  sendPersonaRequest,
...
vi.mock("../src/workflows/helpers/workflowAbort.js", () => ({
  ...
  abortWorkflowWithReason: vi
    .fn()
    .mockResolvedValue({ cleanupResult: { removed: 0, acked: 0 } }),
}));
...
vi.mock("../src/lmstudio.js", () => ({
  callLMStudio: vi.fn().mockResolvedValue({
    content:
      'Generated code response with diffs\n```diff\n--- a/test.ts\n+++ b/test.ts\n@@ -1,1 +1,2 @@\n console.log("hello");\n+console.log("world");\n```',
    raw: {},
  }),
}));
...
describe("Workflow Steps", () => {
  ...
  describe("PullTaskStep", () => {
    ...
    it("should validate configuration", async () => {
      ...
      expect(validation.errors).toContain(
        "PullTaskStep: consumerGroup is required and must be a string",
      );
    });
  });
  ...
  describe("PlanningStep", () => {
    it("should create implementation plans", async () => {
      ...
      vi.mocked(
        await import("../src/lmstudio.js"),
      ).callLMStudio.mockResolvedValueOnce(mockLLMResponse);
  ...
  describe("QAStep", () => {
    it("should execute tests and analyze results", async () => {
      ...
      setTimeout(() => {
        const dataCallback = mockSpawn.stdout.on.mock.calls.find(
          (call) => call[0] === "data",
        )?.[1];
        if (dataCallback) {
  ...
  describe("PlanEvaluationStep", () => {
    ...
    it("should validate configuration properly", async () => {
      ...
      expect(
        validation.errors.some((e) => e.includes("minFeasibilityScore")),
      ).toBe(true);
      expect(validation.errors.some((e) => e.includes("minQualityScore"))).toBe(
        true,
      );
  ...
  describe("QAAnalysisStep", () => {
    ...
    it("should analyze QA results successfully", async () => {
      ...
      expect(result.status).toBe("success");
      expect(result.outputs?.overallStatus).toBeOneOf([
        "critical",
        "concerning",
        "manageable",
        "good",
    ...
    it("should validate configuration", async () => {
      ...
      expect(validation.valid).toBe(false);
      expect(
        validation.errors.some((e) => e.includes("maxFailuresToAnalyze")),
      ).toBe(true);
      expect(
        validation.errors.some((e) => e.includes("Custom categories")),
      ).toBe(true);
    });
  ...
  describe("GitOperationStep", () => {
    it("should abort workflow when working tree is dirty before checkout", async () => {
      ...
      const describeSpy = vi
        .spyOn(gitUtils, "describeWorkingTree")
        .mockResolvedValue({
          dirty: true,
    ...
    it("should abort workflow when push fails", async () => {
      ...
      const commitSpy = vi
        ...
        .mockResolvedValueOnce(commitResult as any);
  ...
  describe("TaskCreationStep", () => {
    ...
    it("should create tasks from QA analysis successfully", async () => {
      ...
      expect(
        tasks.some((t) => t.priority === "critical" || t.priority === "high"),
      ).toBe(true);
    });
    ...
    it("should group related issues when enabled", async () => {
      ...
      expect(tasks.some((t) => t.title && t.title.includes("Type Error"))).toBe(
        true,
      );
    });
    ...
    it("should validate configuration", async () => {
      ...
      expect(
        validation.errors.some((e) => e.includes("minConfidenceThreshold")),
      ).toBe(true);
      expect(
        validation.errors.some((e) => e.includes("title and description")),
      ).toBe(true);
    });
