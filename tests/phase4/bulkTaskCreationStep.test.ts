...
describe('Phase 4 - BulkTaskCreationStep', () => {
  ...
  describe('Day 3: Workflow Abort Signal', () => {
    it('should set abort signal on partial failure with abort_on_partial_failure=true', async () => {
      ...
      expect(context.getVariable('workflow_abort_reason')).toContain('tasks failed after retries');
    });
  ...
  describe('Day 4: Idempotency (external_id)', () => {
    it('should auto-generate external_id when upsert_by_external_id=true', async () => {
      ...
      expect(enrichedTasks[1]?.external_id).toBe('wf-550e8400-e29b:create_tasks_bulk:1');
    });
    ...
    it('should use custom external_id template', async () => {
      const step = new BulkTaskCreationStep({
        ...
        config: {
          ...
          tasks: [
            { title: 'Fix Login Bug', priority: 'critical' as const, milestone_slug: 'sprint-1' },
            { title: 'Add Tests', priority: 'high' as const, milestone_slug: 'sprint-1' }
          ],
          options: {
    ...
    it('should support all template variables', async () => {
      const step = new BulkTaskCreationStep({
        ...
        config: {
          ...
          options: {
            external_id_template: '${workflow_run_id}:${step_name}:${task.priority}:${task.milestone_slug}:${task_index}'
          }
        }
      });
