...
describe('Phase 4 - PMDecisionParserStep', () => {
  ...
  describe('Day 1: Backlog Deprecation & Validation', () => {
    it('should merge backlog and follow_up_tasks arrays', async () => {
      const step = new PMDecisionParserStep({
        ...
        config: {
          input: JSON.stringify({
            ...
            backlog: [
              { title: 'Backlog Task 1', description: 'From backlog', priority: 'low' },
              { title: 'Backlog Task 2', description: 'From backlog', priority: 'medium' }
            ],
            follow_up_tasks: [
              { title: 'Follow-up Task 1', description: 'From follow_up', priority: 'high' }
            ]
          }),
          normalize: true
        }
      ...
      expect(result.outputs?.pm_decision?.follow_up_tasks[0].title).toBe('Follow-up Task 1');
      expect(result.outputs?.pm_decision?.follow_up_tasks[1].title).toBe('Backlog Task 1');
      expect(result.outputs?.pm_decision?.follow_up_tasks[2].title).toBe('Backlog Task 2');
    });
        ...
        config: {
          input: JSON.stringify({
            ...
            follow_up_tasks: [
              ...
              { title: 'High Code', priority: 'high', review_type: 'code_review' },
