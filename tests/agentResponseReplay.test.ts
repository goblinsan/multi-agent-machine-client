...
const fixturePath = path.join(__dirname, 'fixtures/personaOutputs/lead_engineer_previews.json');
...
describe('agent diff replay (lead-engineer persona)', () => {
  for (const testCase of fixture.cases) {
    ...
    it(`parses diff bundle for ${label}`, async () => {
      ...
      expect(parseOutcome.source === 'structured' || parseOutcome.source === 'diff').toBe(true);
    });
  }
});
