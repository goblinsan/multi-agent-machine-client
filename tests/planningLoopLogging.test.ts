...
vi.mock('../src/agents/persona.js', async () => {
  const actual = await vi.importActual<typeof import('../src/agents/persona.js')>('../src/agents/persona.js');
  return {
    ...actual,
...
describe('PlanningLoopStep logging', () => {
  ...
  it('logs plan output and evaluation results for each iteration', async () => {
    ...
    context.setVariable('task', { id: 'task-1', type: 'feature', data: { description: 'Test task' } });
  ...
  const infoSpy = vi.spyOn(logger, 'info');
    ...
    const planLog = infoSpy.mock.calls.find(([message]) => message === 'Planning loop plan output');
    ...
    const evaluationLog = infoSpy.mock.calls.find(([message]) => message === 'Planning loop evaluation result');
