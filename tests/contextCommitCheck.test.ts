...
import {
  isLastCommitContextOnly,
  hasCommitsSinceLastContextScan,
} from "../src/git/contextCommitCheck.js";
...
describe("contextCommitCheck", () => {
  ...
  describe("isLastCommitContextOnly", () => {
    ...
    it("returns false when last commit has both context and code files", async () => {
      ...
      await runGit(["commit", "-m", "feat: add app and context"], {
        cwd: repoRoot,
      });
    ...
    it("returns true when last commit has no files (empty commit)", async () => {
      ...
      await runGit(["commit", "--allow-empty", "-m", "empty commit"], {
        cwd: repoRoot,
      });
  ...
  describe("hasCommitsSinceLastContextScan", () => {
    ...
    it("returns false after updating context scan", async () => {
      ...
      await fs.mkdir(contextDir, { recursive: true });
      await fs.writeFile(
        path.join(contextDir, "snapshot.json"),
        '{"version": 1}',
      );
      await runGit(["add", ".ma/context/"], { cwd: repoRoot });
      await runGit(["commit", "-m", "context: initial scan"], {
        cwd: repoRoot,
      });
      ...
      await runGit(["add", ".ma/context/"], { cwd: repoRoot });
      await runGit(["commit", "-m", "context: updated scan"], {
        cwd: repoRoot,
      });
