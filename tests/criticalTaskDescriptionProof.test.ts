...
describe("PROOF: Planner receives task description or workflow aborts", () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(buildMessagesModule, "callPersonaModel").mockImplementation(
      async (_input: any) => {
        if (llmCallError) {
          throw llmCallError;
        }
        return {
          content: JSON.stringify({
            plan: [
              {
                goal:
                  "Test plan based on: " + capturedUserText.substring(0, 50),
                key_files: ["test.ts"],
                owners: ["dev"],
                dependencies: [],
  ...
  it("PROOF #1: Planner receives task description from task.data.description (dashboard structure)", async () => {
    ...
    console.log(
      "PROOF #1: Task description successfully extracted and sent to LLM",
    );
    console.log(
      "═══════════════════════════════════════════════════════════\n",
    );
    ...
    console.log(
      "✓ Description extracted from task.data.description (dashboard structure)",
    );
  ...
  it("PROOF #2: Workflow aborts with error when task.data.description is missing", async () => {
    ...
    const extractorModule = await import(
      "../src/personas/context/ContextExtractor.js"
    );
    const originalExtractUserText =
      extractorModule.ContextExtractor.prototype.extractUserText;
    vi.spyOn(
      extractorModule.ContextExtractor.prototype,
      "extractUserText",
    ).mockImplementation(async function (this: any, params: any) {
      try {
    ...
    const events = await transport.xRead(
      [{ key: cfg.eventStream, id: "0-0" }],
      { COUNT: 100 },
    );
    ...
    for (const event of eventMessages) {
      ...
      if (
        fields.workflow_id === "proof-test-2" &&
        fields.from_persona === "implementation-planner"
      ) {
        try {
    ...
    console.log(
      ...
      JSON.stringify(errorResultPayload, null, 2),
    );
    ...
    console.log(
      `User text captured: ${capturedUserText ? capturedUserText.substring(0, 100) : "(empty)"}`,
    );
    ...
    console.log(
      "✓ Workflow step will detect failure status and abort workflow",
    );
  ...
  it("PROOF #3: Fallback structure (task.description) still works", async () => {
    ...
    console.log(
      "PROOF #3: Backwards compatibility with task.description structure",
    );
    console.log(
      "═══════════════════════════════════════════════════════════\n",
    );
  ...
  it("PROOF #4: Priority order - task.data.description takes precedence over task.description", async () => {
    ...
    console.log(
      "PROOF #4: task.data.description has priority over task.description",
    );
    console.log(
      "═══════════════════════════════════════════════════════════\n",
    );
    ...
    const dualStructureTask = {
      ...
      data: {
        ...
        title: "Task with both structures",
        description:
          "CORRECT: This is in task.data.description (should be used)",
        status: "open",
      },
      timestamp: Date.now(),
    };
