...
export function annotateCaptured(s: any[]) {
  const stepRole: Record<string, string> = {
    ...
    "3-qa": "QA test run",
    "qa-created-tasks":
      "QA follow-up - created tasks (after summarizer/create)",
...
export function writeCapturedOutputs(
  kind: "default" | "coordinator" = "default",
  extraAnnotate?: (s: any[]) => any[],
) {
  if (!isCaptureEnabled()) return;
  ...
  const jsonPath = path.join(
    outDir,
    kind === "coordinator"
      ? "last_run_prompts_coordinator.json"
      : "last_run_prompts.json",
  );
  try {
    ...
    fs.writeFileSync(
      jsonPath,
      JSON.stringify(
        { generatedAt: new Date().toISOString(), requests: toWrite },
        null,
        2,
      ),
    );
  ...
  if (kind === "default") {
    ...
    lines.push(
      "Each entry below is the raw request object captured for inspection.",
    );
    lines.push("");
    for (const req of extraAnnotate ? extraAnnotate(sent) : sent) {
      lines.push("---");
      lines.push(
        `Phase: ${req.role || req.step || "unknown"} (${req.step || "unknown"})`,
      );
