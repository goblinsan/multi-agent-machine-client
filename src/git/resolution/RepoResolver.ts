...
function repoUrlFromPayload(payload: any): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      logger.debug("Invalid remote URL format", {
        url: trimmed,
        error: String(e),
      });
    }
...
export async function resolveRepoFromPayload(
  payload: any,
  ...
  for (const cand of localPathCandidates) {
    ...
    if (await directoryExists(gitDir).catch(() => false)) {
      if (
        !cfg.allowWorkspaceGit &&
        path.resolve(trimmed) === path.resolve(process.cwd())
      ) {
        if (!remote) {
          throw new Error(
            "Workspace repo provided but workspace mutations are disabled. Provide a repository remote URL so we can clone into PROJECT_BASE, or set MC_ALLOW_WORKSPACE_GIT=1 to opt-in.",
          );
        }
  ...
  if (
    ...
    if (
      ...
      if (!remote) {
        throw new Error(
          "Workspace repo_root provided but workspace mutations are disabled. Provide a repository remote URL so we can clone into PROJECT_BASE, or set MC_ALLOW_WORKSPACE_GIT=1 to opt-in.",
        );
      }
      const ensured = await ensureRepo(remote, branch, hint);
      return {
  ...
  throw new Error(
    "No repository remote provided and repo_root is not a git repository. Configure the project's repository URL in the dashboard or provide a valid repo_root.",
  );
}
