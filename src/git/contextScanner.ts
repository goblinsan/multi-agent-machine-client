...
export async function scanRepositoryForContext(
  ...
  const componentsInput = Array.isArray(opts.components)
    ? opts.components
    : null;
  const comps: Comp[] =
    componentsInput && componentsInput.length
  ...
  for (const c of comps) {
    ...
    perComp.push({
      component: compName,
      totals: sum.totals,
      largest: sum.largest.slice(0, 10),
      longest: sum.longest.slice(0, 10),
    });
  }
...
function buildScanMarkdown(
  ...
  for (const pc of perComp) {
    ...
    for (const f of pc.longest)
      lines.push(`  - ${f.path} (${f.lines || 0} lines)`);
    lines.push("");
  }
  ...
  const alembicFiles = allFiles.filter((f) =>
    /(^|\/)alembic(\/|$)/i.test(f.path),
  );
  if (alembicFiles.length) {
    const versions = alembicFiles.filter((f) =>
      /(^|\/)alembic(\/|).*\bversions\b(\/|).+\.py$/i.test(f.path),
    );
    ...
    lines.push(
      `- Alembic tree detected (files: ${alembicFiles.length}, versions: ${versions.length})`,
    );
    lines.push(
      versions.length
        ? "- Latest versions (by modified time):"
        : "- No versioned migrations found under alembic/versions",
    );
    for (const f of latest) {
      lines.push(
        `  - ${f.path}  (mtime=${new Date(f.mtime).toISOString()}, bytes=${f.bytes}${typeof f.lines === "number" ? `, lines=${f.lines}` : ""})`,
      );
    }
    lines.push("");
  }
