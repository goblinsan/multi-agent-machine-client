...
async function main() {
  ...
  const personasWithoutModels = cfg.allowedPersonas.filter(
    (p) => !cfg.personaModels[p],
  );
  if (personasWithoutModels.length > 0) {
    console.warn(
      "WARNING: Some personas have no model mapping:",
      personasWithoutModels.join(", "),
    );
