...
export function registerMilestoneRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request: any, reply: any) => {
      ...
      const projectResult = db.exec("SELECT id FROM projects WHERE id = ?", [
        projectId,
      ]);
      ...
      const result = db.exec(
        "SELECT * FROM milestones WHERE id = last_insert_rowid()",
      );
      saveDb(db);
  ...
  fastify.patch(
    ...
    async (request: any, reply: any) => {
      ...
      const result = db.exec("SELECT * FROM milestones WHERE id = ?", [
        parseInt(id),
      ]);
      const cols = result[0].columns;
      const row = result[0].values[0];
