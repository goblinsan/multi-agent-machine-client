        ...
        for (const t of body.tasks) {
          ...
          if (parsed.external_id) {
            ...
            if (existingResult[0] && existingResult[0].values.length > 0) {
              ...
              if (existing.labels)
                existing.labels = JSON.parse(existing.labels);
  ...
  fastify.patch(
    ...
    async (request: any, reply: any) => {
      ...
      if (payload.priority_score !== undefined) {
        ...
        params.push(payload.priority_score);
      }
      if (payload.milestone_id !== undefined) {
        updates.push("milestone_id = ?");
        params.push(payload.milestone_id);
      }
