...
export function registerProjectRoutes(fastify: FastifyInstance) {
  fastify.get("/projects", async (_request: any, _reply: any) => {
    ...
    const result = db.exec(
      "SELECT id, name, slug, description, created_at, updated_at FROM projects ORDER BY created_at DESC LIMIT 100",
    );
  ...
  fastify.post("/projects", async (request: any, reply: any) => {
    ...
    const existingResult = db.exec("SELECT id FROM projects WHERE slug = ?", [
      data.slug,
    ]);
    ...
    db.run("INSERT INTO projects (name, slug, description) VALUES (?, ?, ?)", [
      data.name,
      data.slug,
  ...
  fastify.patch("/projects/:id", async (request: any, reply: any) => {
    ...
    const existingResult = db.exec("SELECT id FROM projects WHERE id = ?", [
      id,
    ]);
    if (!existingResult[0] || existingResult[0].values.length === 0) {
      return reply.status(404).send({
  ...
  fastify.get("/projects/:id/status", async (request: any, reply: any) => {
    ...
    const repositories = repoResult[0]
      ? repoResult[0].values.map((row: any) => ({
          id: row[0],
          url: row[1],
    ...
    const milestones = milestoneResult[0]
      ? milestoneResult[0].values.map((row: any) => ({
          id: row[0],
          name: row[1],
          slug: row[2],
          status: row[3],
