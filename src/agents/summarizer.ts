...
export async function summarizeTask(
  ...
  try {
    await sendPersonaRequest(r, {
      ...
      projectId:
        task && (task.project_id || task.projectId)
    ...
    const summary =
      res?.payload?.summary ?? res?.payload?.condensed ?? res?.payload ?? res;
    ...
    logger.warn("summarizeTask failed, falling back to original description", {
      err,
      task,
    });
