...
export async function fetchContext(workflowId: string) {
  try {
    if (
      ...
      cfg.dashboardContextEndpoint.startsWith("http")
    ) {
    ...
    const r = await fetch(
      `${cfg.dashboardBaseUrl.replace(/\/$/, "")}/context/by-workflow?workflow_id=${encodeURIComponent(workflowId)}&limit=5`,
      {
        headers: { Authorization: `Bearer ${cfg.dashboardApiKey}` },
      },
    );
...
export async function fetchProjectStatusDetails(
  projectId: string | null | undefined,
) {
  return projectAPI.fetchProjectStatusDetails(projectId);
}

export async function fetchProjectStatusSummary(
  projectId: string | null | undefined,
) {
