...
export type FileInfo = { path: string; bytes: number; lines?: number; sha1?: string; mtime: number };
  ...
  async function walk(dir: string, depth: number) {
    ...
    for (const e of ents) {
      ...
      if (e.isDirectory()) {
        ...
        bytesSeen += stat.size;
        if (results.length >= spec.max_files || bytesSeen > spec.max_bytes) return;
        const fi: FileInfo = { path: relUnix, bytes: stat.size, mtime: stat.mtimeMs };
...
export function summarize(files: FileInfo[]) {
  const totals = files.reduce((a, f) => ({ files: a.files + 1, bytes: a.bytes + f.bytes, lines: a.lines + (f.lines || 0) }), { files: 0, bytes: 0, lines: 0 });
  const largest = [...files].sort((a,b)=>b.bytes-a.bytes).slice(0, 20);
  const longest = [...files].filter(f => typeof f.lines === "number").sort((a,b)=>(b.lines||0)-(a.lines||0)).slice(0, 20);
