...
export class BlockedTaskAnalysisStep extends WorkflowStep {
    ...
    try {
      ...
      if (workflowError) {
        analysis.error_message =
          analysis.error_message || String(workflowError);
      }
  ...
  private generateContextHints(analysis: BlockageAnalysis): string[] {
    ...
    if (
      analysis.failed_step?.includes("push") ||
      analysis.error_message?.includes("push")
    ) {
    ...
    if (analysis.error_message?.includes("timeout")) {
      hints.push(
        "Operation timed out - may need longer timeout or optimization",
      );
    }
    ...
    if (analysis.previous_attempts.length > 0) {
      ...
      const sameError = recentFailures.every(
        (a) => a.error === analysis.error_message,
      );

      if (sameError) {
        hints.push(
          "Same error repeated multiple times - may need different approach",
        );
