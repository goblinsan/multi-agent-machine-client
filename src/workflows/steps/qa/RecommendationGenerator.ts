...
export class RecommendationGenerator {
  generateRecommendations(
  ...
  ): QAAnalysisResult["recommendations"] {
    const recommendations: QAAnalysisResult["recommendations"] = [];

    const criticalFailures = analysis.failureAnalyses.filter(
      (f) => f.severity === "high",
    );
    ...
    const timeoutFailures = analysis.failureAnalyses.filter(
      (f) => f.pattern === "timeout",
    );
  ...
  generateNextActions(
    ...
    if (qaResults.status === "failed") {
      const criticalIssues = analysis.failureAnalyses.filter(
        (f) => f.severity === "high",
      ).length;
      const mediumIssues = analysis.failureAnalyses.filter(
        (f) => f.severity === "medium",
      ).length;

      if (criticalIssues > 0) {
        actions.push(
          `Fix ${criticalIssues} critical error(s) preventing basic functionality`,
        );
      }

      if (mediumIssues > 5) {
        actions.push(
          "Address systematic issues causing multiple medium-severity failures",
        );
      ...
      if (topCategory && topCategory.count > 1) {
        actions.push(
          `Focus on ${topCategory.name} issues (${topCategory.count} occurrences)`,
        );
      }
    } else {
      actions.push(
        "All tests passing - consider expanding test coverage or adding edge cases",
      );
    }
  ...
  assessOverallStatus(
    qaResults: QAResults,
  ): QAAnalysisResult["overallAssessment"] {
    const failureRate = qaResults.failedTests / qaResults.totalTests;
