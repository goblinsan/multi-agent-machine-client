...
export class ReviewFailureTasksStep extends WorkflowStep {
  ...
  async execute(context: WorkflowContext): Promise<StepResult> {
    ...
    try {
      ...
      if (!pmDecision) {
        logger.error(
          "No PM decision found in context - ensure PMDecisionParserStep runs before this step",
          {
            stepName: this.config.name,
        ...
        return {
          ...
          error: new Error(
            `Missing PM decision variable: ${config.pmDecisionVariable}. Ensure PMDecisionParserStep runs first.`,
          ),
          metrics: {
      ...
      if (
        !pmDecision.follow_up_tasks ||
        !Array.isArray(pmDecision.follow_up_tasks)
      ) {
        logger.error(
          "PM decision missing follow_up_tasks array - expected normalized output from PMDecisionParserStep",
          {
            stepName: this.config.name,
        ...
        return {
          status: "failure" as const,
          error: new Error(
            "Invalid PM decision format - follow_up_tasks array required. Ensure PMDecisionParserStep normalization is enabled.",
          ),
          metrics: {
            duration_ms: Date.now() - startTime,
          },
      ...
      if (pmDecision.follow_up_tasks.length > 0) {
        for (const followUpTask of pmDecision.follow_up_tasks) {
          const isUrgent = ["critical", "high"].includes(
            followUpTask.priority?.toLowerCase() || "",
          );

          try {
            const defaultUrgentPriority =
              config.reviewType === "qa" ? 1200 : 1000;
            const priorityScore = isUrgent
              ? config.urgentPriorityScore || defaultUrgentPriority
              : config.deferredPriorityScore || 50;

            const taskTitle = this.formatTaskTitle(
              followUpTask.title,
              config.reviewType,
              isUrgent,
            );
            ...
            const targetMilestoneSlug = isUrgent
              ? undefined
  ...
  async validate(_context: WorkflowContext): Promise<ValidationResult> {
    ...
    if (!config.reviewType) {
      ...
      !["code_review", "security_review", "qa", "devops"].includes(
        config.reviewType,
      )
    ) {
      errors.push(
        "ReviewFailureTasksStep: reviewType must be one of: code_review, security_review, qa, devops",
      );
    }

    if (
      config.urgentPriorityScore !== undefined &&
      config.urgentPriorityScore < 0
    ) {
      errors.push(
        "ReviewFailureTasksStep: urgentPriorityScore must be non-negative",
      );
    }

    if (
      config.deferredPriorityScore !== undefined &&
      config.deferredPriorityScore < 0
    ) {
      errors.push(
        "ReviewFailureTasksStep: deferredPriorityScore must be non-negative",
      );
    }

    if (
      config.pmDecisionVariable &&
      !config.pmDecisionVariable.includes("pm_decision") &&
      !config.pmDecisionVariable.includes("parsed_decision")
    ) {
      warnings.push(
        `ReviewFailureTasksStep: pmDecisionVariable "${config.pmDecisionVariable}" should typically be "pm_decision" or "parsed_decision" from PMDecisionParserStep`,
      );
    }
