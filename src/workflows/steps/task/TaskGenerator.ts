...
export class TaskGenerator {
  private static readonly DEFAULT_TEMPLATES = {
    ...
    'coverage-gap': {
      ...
      description: 'Coverage gap detected in {component}\n\nCurrent coverage: {coverage}%\nTarget: 80%',
    ...
    if (failures && Array.isArray(failures)) {
      for (let i = 0; i < failures.length; i++) {
        ...
        const task: TaskDefinition = {
          ...
          labels: ['qa', failure.category || 'test', failure.severity || 'medium'],
          sourceData: {
            type: 'qa-failure',
            sourceId: taskId,
            confidence: failure.confidence || 0.7
    ...
    if (evaluation.issues && Array.isArray(evaluation.issues)) {
      for (let i = 0; i < evaluation.issues.length; i++) {
        ...
        const task: TaskDefinition = {
          ...
          priority: this.mapSeverityToPriority(issue.severity || issue.priority || 'medium'),
        ...
        if (issue.blockedBy) {
          task.dependencies = Array.isArray(issue.blockedBy) ? issue.blockedBy : [issue.blockedBy];
        }
