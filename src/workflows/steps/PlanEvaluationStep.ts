...
export class PlanEvaluationStep extends WorkflowStep {
    ...
    try {
      ...
      return {
        ...
        metrics: {
          ...
          operations_count: evaluation.issues.length + evaluation.recommendations.length
        }
      };
  ...
  protected async validateConfig(_context: WorkflowContext): Promise<ValidationResult> {
    ...
    if (config.minFeasibilityScore !== undefined && (config.minFeasibilityScore < 0 || config.minFeasibilityScore > 1)) {
      errors.push('PlanEvaluationStep: minFeasibilityScore must be between 0 and 1');
    }
    
    if (config.minQualityScore !== undefined && (config.minQualityScore < 0 || config.minQualityScore > 1)) {
      errors.push('PlanEvaluationStep: minQualityScore must be between 0 and 1');
    }
    
    if (config.maxComplexityScore !== undefined && (config.maxComplexityScore < 0 || config.maxComplexityScore > 1)) {
      errors.push('PlanEvaluationStep: maxComplexityScore must be between 0 and 1');
    }
    
    if (config.customCriteria) {
      for (const criteria of config.customCriteria) {
        if (!criteria.name || !criteria.description) {
          errors.push('PlanEvaluationStep: Custom criteria must have name and description');
        }
        if (criteria.weight < 0 || criteria.weight > 1) {
          errors.push('PlanEvaluationStep: Custom criteria weight must be between 0 and 1');
        }
      }
    }
  ...
  private extractPlanData(context: WorkflowContext, config: PlanEvaluationConfig): PlanData | null {
  ...
  private evaluatePlan(planData: PlanData, config: PlanEvaluationConfig): EvaluationResult {
  ...
  private evaluateFeasibility(planData: PlanData, issues: EvaluationResult['issues'], recommendations: string[]): number {
    ...
    if (planData.timeline) {
      ...
      if (planData.timeline.confidence === 'low') {
        recommendations.push('Consider breaking down the plan into smaller, more predictable chunks');
        score -= 0.05;
      }
    }
  ...
  private evaluateQuality(planData: PlanData, issues: EvaluationResult['issues'], recommendations: string[]): number {
    ...
    if (planData.steps) {
      ...
      if (rationaleRatio < 0.5) {
        recommendations.push('Add rationale explanations for more implementation steps');
        score -= 0.1;
      }
    }
  ...
  private evaluateComplexity(planData: PlanData, issues: EvaluationResult['issues'], recommendations: string[]): number {
    ...
    if (complexityScore > 0.7) {
      recommendations.push('Consider breaking this plan into smaller, less complex phases');
    }
  ...
  private evaluateRiskAssessment(planData: PlanData, config: PlanEvaluationConfig, issues: EvaluationResult['issues'], recommendations: string[]): number {
    let score = 1.0;
    ...
    if (config.requireRiskAssessment) {
      ...
      recommendations.push('Consider adding risk assessment to improve plan robustness');
      score -= 0.1;
    }
  ...
  private isPlanApproved(evaluation: EvaluationResult, config: PlanEvaluationConfig): boolean {
