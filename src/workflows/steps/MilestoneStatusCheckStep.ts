  ...
  async execute(context: WorkflowContext): Promise<StepResult> {
    ...
    try {
      ...
      const incompleteTasks = milestoneTasks.filter((t: any) => {
        ...
        return !completeStatuses.includes(status) && !completeStatuses.includes(normalizedStatus);
      });
  ...
  protected async validateConfig(context: WorkflowContext): Promise<ValidationResult> {
    ...
    if (config.check_type) {
      ...
      if (!validTypes.includes(config.check_type)) {
        errors.push(`Invalid check_type: ${config.check_type}. Must be one of: ${validTypes.join(', ')}`);
      }
    }
