    ...
    try {
      ...
      for (const file of yamlFiles) {
        ...
        try {
          ...
          console.warn(
            `Failed to load workflow from ${file}: ${error.message}`,
          );
        }
      }
      ...
      throw new Error(
        `Failed to load workflows from directory ${directoryPath}: ${error.message}`,
      );
    }
  }
  ...
  private validateWorkflowDefinition(definition: WorkflowDefinition): void {
    ...
    for (const step of definition.steps) {
      ...
      if (!hasTemplate && !this.stepRegistry.has(step.type)) {
        throw new Error(
          `Unknown step type '${step.type}' in step '${step.name}'`,
        );
      }
    }
