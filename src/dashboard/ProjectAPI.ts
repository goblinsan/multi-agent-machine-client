...
export class ProjectAPI extends DashboardClient {
  async fetchProjectStatus(
    ...
    try {
      ...
      logger.warn("fetchProjectStatus failed", {
        projectId,
  ...
  async fetchProjectMilestones(
    ...
    try {
      ...
      logger.warn("fetchProjectMilestones failed", {
        projectId,
        error: (e as Error).message,
      });
      return null;
  ...
  async fetchProjectStatusDetails(
    projectId: string | null | undefined,
  ): Promise<any | null> {
    if (!projectId) return null;
    ...
    try {
      ...
      logger.warn("fetchProjectStatusDetails failed", {
        projectId,
        error: (e as Error).message,
      });
      return null;
  ...
  async fetchProjectStatusSummary(
    projectId: string | null | undefined,
  ): Promise<string | null> {
    if (!projectId) return null;
    ...
    try {
      const data = await this.get(
        `/projects/${encodeURIComponent(projectId)}/status/summary`,
      );
      if (!data) return null;
      ...
      logger.warn("fetchProjectStatusSummary failed", {
        projectId,
        error: (e as Error).message,
      });
      return null;
  ...
  async fetchProjectTasks(projectId: string): Promise<any[]> {
    ...
    try {
      ...
      logger.warn("fetchProjectTasks exception", {
        projectId,
        error: (e as Error).message,
      });
      return [];
