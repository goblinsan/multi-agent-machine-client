...
export async function cleanupTaskLogs(options: {
  ...
  try {
    ...
    const qaLogPath = pathMod.resolve(
      repoRoot,
      ".ma/qa",
      `task-${taskId}-qa.log`,
    );
    const planLogPath = pathMod.resolve(
      repoRoot,
      ".ma/planning",
      `task-${taskId}-plan.log`,
    );
    ...
    try {
      ...
      logger.info("Updated changelog with task summary", {
        taskId,
        changelogPath,
      });
    } catch (e: any) {
...
function generateTaskSummary(
  ...
  if (qaContent) {
    ...
    if (finalStatus === "FAIL") {
      lines.push(
        "- Note: Task marked complete despite failing QA (may need follow-up)",
      );
    }
    lines.push("");
  }
