...
async function testDashboardIntegration() {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Dashboard backend not responding. Is it running on port 8080?",
      );
    }
    ...
    console.log(
      "   Task IDs:",
      bulkResponse.created.map((t) => t.id),
    );
    ...
    console.log(
      "✅ First bulk create - Created:",
      bulkResponse1.created.length,
    );
    ...
    console.log(
      "✅ Second bulk create - Created:",
      bulkResponse2.created.length,
    );
    console.log(
      "✅ Second bulk create - Skipped:",
      bulkResponse2.skipped?.length || 0,
    );

    if (
      bulkResponse2.created.length === 0 &&
      bulkResponse2.skipped?.length === 3
    ) {
      console.log("✅ Bulk idempotency works! All tasks skipped.");
    } else {
      console.log(
        "❌ Bulk idempotency issue! Created:",
        bulkResponse2.created.length,
        "Skipped:",
        bulkResponse2.skipped?.length,
      );
    }
